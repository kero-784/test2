...
function renderTasks() {
    ...
    if (tasksToDisplay.length === 0) {
        taskListContainer.innerHTML = '<div class="alert alert-light">No tasks to display.</div>'; return;
    }
    const branchMap = Object.fromEntries(APP_DATA.branches.map(b => [b.id, b.name]));
...
function renderReport(reportData) {
    ...
    outputDiv.innerHTML = `<div class="card" id="report-card-export"><div class="card-header d-flex justify-content-between align-items-center"><h4>${reportData.title}</h4><div><button class="btn btn-success me-2" onclick='exportToExcel(${JSON.stringify(reportData)})'><i class="bi bi-file-earmark-excel"></i> Excel</button><button class="btn btn-info" onclick="exportReportToJpg()"><i class="bi bi-file-earmark-image"></i> JPG</button></div></div><div class="card-body"><div class="row"><div class="col-md-7"><h5>Data Table</h5><div class="table-responsive"><table class="table table-striped table-bordered"><thead><tr>${headers.map(h => `<th>${h}</th>`).join("")}</tr></thead><tbody>${reportData.table.map(row => `<tr>${headers.map(h => `<td>${row[h]}</td>`).join("")}</tr>`).join("")}</tbody></table></div></div><div class="col-md-5"><h5>Chart</h5><canvas id="report-chart-canvas"></canvas></div></div></div></div>`;

    if (reportChart) reportChart.destroy();

    const chartColors = {
        pie: ["#10b981", "#f59e0b", "#a5b4fc"], // Green, Amber
...
// NEW: Robust Excel Export
function exportToExcel(reportData) {
    showToast("Preparing your Excel file. This may take a moment.", "success");
...
// NEW: Full Report JPG Export
function exportReportToJpg() {
    const reportCard = document.getElementById("report-card-export");
    if (!reportCard) { showToast("No report to export.", "danger"); return; }
    showToast("Generating JPG, please wait...", "success");

    // Temporarily apply a class for rendering, then use html2canvas
    reportCard.classList.add("print-view");
    html2canvas(reportCard, { scale: 2 }) // Higher scale for better quality
        .then(canvas => {
            reportCard.classList.remove("print-view"); // Remove class after rendering
        ...
        .catch(err => {
            reportCard.classList.remove("print-view"); // Ensure class is removed on error
...
function cancelEditEntity(btn){const li=btn.closest(".list-grom up-item");li.classList.remove("editing")}
