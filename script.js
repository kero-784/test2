...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 4. BUSINESS LOGIC (Backup, Restore, Payment, Submission) ---
    ...
    async function loadAutoBackupSettings() {
        ...
        if (result && result.data.enabled !== undefined) {
            ...
            toggle.checked = isEnabled;
            frequencyContainer.style.display = isEnabled ? 'block' : 'none';
            statusEl.textContent = isEnabled ? 'Automatic backups active.' : 'Automatic backups disabled.';
        } else {
            statusEl.textContent = 'Could not retrieve status.';
        }
    }
    ...
    async function loadAndRenderBackups() {
        ...
        if (result && result.data) {
            ...
            if (state.backups.length === 0) { container.innerHTML = `<p>${_t('no_backups_found')}</p>`; return; }
            ...
            state.backups.forEach(backup => {
                tableHtml += `<tr><td>${backup.name}</td><td>${new Date(backup.dateCreated).toLocaleString()}</td><td><div class="action-buttons"><a href="${backup.url}" target="_blank" class="secondary small">${_t('open')}</a><button class="danger small btn-restore" data-url="${backup.url}">${_t('restore')}</button></div></td></tr>`;
            });
            tableHtml += '</tbody></table>';
            container.innerHTML = tableHtml;
        } else { container.innerHTML = `<p>Could not load backup list.</p>`; }
    }
    ...
    async function handleConfirmRestore(e) {
        ...
        if (selectedSheets.length === 0) { showToast(_t('restore_select_sheet_toast'), 'error'); return; }
        const payload = { backupFileId: backupFileId, sheetsToRestore: selectedSheets };
    ...
    async function handleUpdateSubmit(e) {
        ...
        let payload = {}, action;
        if (type === 'item') {
            action = 'updateData'; const updates = {};
            ...
            action = id ? 'updateUser' : 'addUser'; payload = {};
            ...
            action = 'updateRolePermissions'; const updates = {};
        ...
        if (result) {
            showToast(id ? _t('update_success_toast', {type}) : _t('add_success_toast', {type}), 'success');
            closeModal();
            await reloadDataAndRefreshUI();
        }
    }
    ...
    async function handleTransactionSubmit(payload, buttonEl) {
        ...
        if (result) {
            ...
            if (payload.type === 'receive' || payload.type === 'po') message = _t('tx_processed_approval_toast', { txType: _t(typeKey) });
    ...
    // --- 5. RENDERERS (DEFINED BEFORE USE) ---

    function updateReceiveGrandTotal() { let grandTotal = 0; (state.currentReceiveList || []).forEach(item => { if(item.isMainItemPlaceholder) { grandTotal += (parseFloat(item.cost) || 0); } else if(!findByKey(state.items, 'code', item.itemCode)?.ParentItemCode) { grandTotal += (parseFloat(item.quantity) || 0) * (parseFloat(item.cost) || 0); } }); document.getElementById('receive-grand-total').textContent = `${grandTotal.toFixed(2)} EGP`; }
    ...
    function updatePOGrandTotal() { let grandTotal = 0; (state.currentPOList || []).forEach(item => { if(item.isMainItemPlaceholder) { grandTotal += (parseFloat(item.cost) || 0); } else if(!findByKey(state.items, 'code', item.itemCode)?.ParentItemCode) { grandTotal += (parseFloat(item.quantity) || 0) * (parseFloat(item.cost) || 0); } }); document.getElementById('po-grand-total').textContent = `${grandTotal.toFixed(2)} EGP`; }
    function updatePOEditGrandTotal() { let grandTotal = 0; (state.currentEditingPOList || []).forEach(item => { if(!findByKey(state.items, 'code', item.itemCode)?.ParentItemCode) { grandTotal += (parseFloat(item.quantity) || 0) * (parseFloat(item.cost) || 0); } }); document.getElementById('edit-po-grand-total').textContent = `${grandTotal.toFixed(2)} EGP`; }
    function updateReturnGrandTotal() { let grandTotal = 0; (state.currentReturnList || []).forEach(item => { if(!findByKey(state.items, 'code', item.itemCode)?.ParentItemCode) { grandTotal += (parseFloat(item.quantity) || 0) * (parseFloat(item.cost) || 0); } }); document.getElementById('return-grand-total').textContent = `${grandTotal.toFixed(2)} EGP`; }
    ...
    const renderDynamicListTable = (tbodyId, list, columnsConfig, emptyMessage, totalizerFn) => {
        ...
        tbody.innerHTML = '';
        if (!list || list.length === 0) { tbody.innerHTML = `<tr><td colspan="${columnsConfig.length + 1}" style="text-align:center;">${_t(emptyMessage)}</td></tr>`; if (totalizerFn) totalizerFn(); return; }
        ...
        allGroups.sort((a, b) => { const idxA = a.parentIndex > -1 ? a.parentIndex : (a.children[0]?.index || 9999); const idxB = b.parentIndex > -1 ? b.parentIndex : (b.children[0]?.index || 9999); return idxA - idxB; });

        allGroups.forEach(group => {
            if (!group.parentItem && group.children.length === 0) return;
            if (group.parentItem) {
                const parentItem = group.parentItem; const parentIndex = group.parentIndex; const isMainPlaceholder = parentItem.isMainItemPlaceholder;
                let totalSubItemWeight = 0;
                if (isMainPlaceholder) { totalSubItemWeight = group.children.reduce((sum, entry) => sum + (parseFloat(entry.item.quantity) || 0), 0); }
                const tr = document.createElement('tr'); if (isMainPlaceholder) tr.classList.add('main-item-group-header');
                let cellsHtml = '';
                const currentItemQty = isMainPlaceholder ? totalSubItemWeight : (parseFloat(parentItem.quantity) || 0);
                const currentItemCost = parseFloat(parentItem.cost) || 0;
                columnsConfig.forEach(col => {
                    let content = '';
                    switch (col.type) {
                        case 'text': content = parentItem[col.key]; break;
                        case 'number_input':
                            const readOnlyAttr = isMainPlaceholder ? 'readonly' : '';
                            const valueDisplay = isMainPlaceholder ? currentItemQty.toFixed(2) : (parentItem.quantity || '');
                            content = `<input type="number" class="table-input" value="${valueDisplay}" min="${col.min || 0.01}" ${col.maxKey ? `max="${(stock[branchCode]?.[parentItem.itemCode]?.quantity || 0)}"` : ''} step="0.01" data-index="${parentIndex}" data-field="${col.key}" ${readOnlyAttr}>`;
                            break;
                        case 'cost_input':
                            if (isMainPlaceholder) content = `<input type="number" class="table-input" value="${currentItemCost.toFixed(2)}" min="0" step="0.01" data-index="${parentIndex}" data-field="cost" title="Enter Total Batch Value">`;
                            else if (findByKey(state.items, 'code', parentItem.itemCode)?.ParentItemCode) content = '---'; 
                            else content = `<input type="number" class="table-input" value="${currentItemCost.toFixed(2)}" min="0" step="0.01" data-index="${parentIndex}" data-field="cost">`;
                            break;
                        case 'calculated': 
                            let calculatedValue = isMainPlaceholder ? currentItemCost : (currentItemQty * currentItemCost);
                            content = `<span>${calculatedValue.toFixed(2)} EGP</span>`;
                            break;
                        case 'available_stock': content = (stock[branchCode]?.[parentItem.itemCode]?.quantity || 0).toFixed(2); break;
                    }
                    cellsHtml += `<td>${content}</td>`;
                });
                cellsHtml += `<td>${!isMainPlaceholder ? `<button class="danger small" data-index="${parentIndex}">X</button>` : '---'}</td>`;
                tr.innerHTML = cellsHtml; tbody.appendChild(tr);
            }
            group.children.forEach(entry => {
                const subItem = entry.item; const subIndex = entry.index;
                const subTr = document.createElement('tr'); subTr.classList.add('sub-item-row');
                let subCellsHtml = '';
                columnsConfig.forEach(col => {
                    let subContent = '';
                    switch (col.type) {
                        ...
                        case 'number_input': 
                            ...
                            break;
                        case 'cost_input': case 'calculated': subContent = '---'; break;
                        case 'available_stock': subContent = (stock[branchCode]?.[subItem.itemCode]?.quantity || 0).toFixed(2); break;
                    }
                    subCellsHtml += `<td>${subContent}</td>`;
                });
                subCellsHtml += `<td><button class="danger small" data-index="${subIndex}">X</button></td>`;
                subTr.innerHTML = subCellsHtml; tbody.appendChild(subTr);
            });
        });
    ...
    function renderAdjustmentListTable() {
        ...
        if (!state.currentAdjustmentList || state.currentAdjustmentList.length === 0) { tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">${_t('no_items_for_adjustment')}</td></tr>`; return; }
        const stock = calculateStockLevels(); const branchCode = document.getElementById('adjustment-branch').value;
        state.currentAdjustmentList.forEach((item, index) => {
            const systemQty = (stock[branchCode]?.[item.itemCode]?.quantity) || 0; const physicalCount = item.physicalCount !== undefined ? item.physicalCount : ''; const adjustment = (parseFloat(physicalCount)||0) - systemQty; item.physicalCount = physicalCount;
            tbody.innerHTML += `<tr><td>${item.itemCode}</td><td>${item.itemName}</td><td>${systemQty.toFixed(2)}</td><td><input type="number" class="table-input" value="${physicalCount}" min="0" step="0.01" data-index="${index}" data-field="physicalCount"></td><td style="font-weight:bold;">${adjustment.toFixed(2)}</td><td><button class="danger small" data-index="${index}">X</button></td></tr>`;
        });
    }
    function renderTransactionHistory(filters = {}) {
        const tbody = document.getElementById('table-transaction-history').querySelector('tbody'); tbody.innerHTML = '';
        let allTx = [...state.transactions]; let allPo = [...state.purchaseOrders];
        ...
        const grouped = {}; allHistoryItems.forEach(t => { if(!grouped[t.batchId]) grouped[t.batchId] = []; grouped[t.batchId].push(t); });
        Object.values(grouped).sort((a,b) => new Date(b[0].date) - new Date(a[0].date)).forEach(group => {
            const first = group[0];
            tbody.innerHTML += `<tr><td>${new Date(first.date).toLocaleString()}</td><td>${first.type}</td><td>${first.batchId}</td><td>${group.length} Items</td><td>${first.Status}</td><td><button class="secondary small btn-view-tx" data-batch-id="${first.batchId}" data-type="${first.type}">${_t('view_print')}</button></td></tr>`;
        });
    }
    function renderItemsTable() { const tbody = document.getElementById('table-items').querySelector('tbody'); tbody.innerHTML = ''; (state.items||[]).forEach(i => tbody.innerHTML += `<tr><td>${i.code}</td><td>${i.name}</td><td>${i.ParentItemCode||''}</td><td>${i.cost}</td><td><button class="secondary small btn-edit" data-type="item" data-id="${i.code}">Edit</button></td></tr>`); }
    function renderSuppliersTable() { const tbody = document.getElementById('table-suppliers').querySelector('tbody'); tbody.innerHTML = ''; (state.suppliers||[]).forEach(s => tbody.innerHTML += `<tr><td>${s.supplierCode}</td><td>${s.name}</td><td>-</td><td><button class="secondary small btn-edit" data-type="supplier" data-id="${s.supplierCode}">Edit</button></td></tr>`); }
    function renderBranchesTable() { const tbody = document.getElementById('table-branches').querySelector('tbody'); tbody.innerHTML = ''; (state.branches||[]).forEach(b => tbody.innerHTML += `<tr><td>${b.branchCode}</td><td>${b.branchName}</td><td><button class="secondary small btn-edit" data-type="branch" data-id="${b.branchCode}">Edit</button></td></tr>`); }
    ...
    function showView(viewId, subviewId = null) {
        ...
        if (targetView && targetLink) {
            ...
            if (subviewId) {
                const subview = targetView.querySelector(`[data-subview="${subviewId}"]`);
                if (subview) subview.click();
            }
    ...
    function openItemSelectorModal(e) {
        modalContext = e.target.closest('button').dataset.context;
        ...
        filteredItems.forEach(item => {
            ...
            itemDiv.innerHTML = `
                ...
                <label for="modal-item-${item.code}">
                    ...
                    ${item.cost ? `<br><small style="color:var(--text-light-color)">Cost: ${item.cost} EGP</small>` : ''}
                </label>
            `;
    ...
    function confirmModalSelection() {
        ...
        checkboxes.forEach(cb => {
            const itemCode = cb.dataset.code || cb.dataset.number;
            if (modal.id === 'item-selector-modal') {
                state.modalSelections.add(itemCode);
            } else if (modal.id === 'invoice-selector-modal') {
                state.invoiceModalSelections.add(itemCode);
            }
        });
        ...
        if (modal.id === 'item-selector-modal') {
            addSelectedItemsToCurrentList();
        } else if (modal.id === 'invoice-selector-modal') {
            renderPaymentList();
        }
    }

    function addSelectedItemsToCurrentList() {
        state.modalSelections.forEach(itemCode => {
            const item = findByKey(state.items, 'code', itemCode);
            if (!item) return;
            
            const existingIndex = state[`current${modalContext.charAt(0).toUpperCase() + modalContext.slice(1)}List`]
                .findIndex(i => i.itemCode === itemCode);
            ...
            if (existingIndex === -1) {
                ...
                if (modalContext === 'POList' && item.ParentItemCode) {
                    newItem.cost = 0;
                }
                
                state[`current${modalContext.charAt(0).toUpperCase() + modalContext.slice(1)}List`].push(newItem);
            }
        });
        ...
        switch(modalContext) {
            case 'ReceiveList': renderReceiveListTable(); break;
            case 'TransferList': renderTransferListTable(); break;
            case 'POList': renderPOListTable(); break;
            case 'ReturnList': renderReturnListTable(); break;
            case 'AdjustmentList': 
                state.currentAdjustmentList = state.currentAdjustmentList || [];
    ...
    function setupInputTableListeners(tableId, listProperty, renderFn) {
        ...
        tbody.addEventListener('input', e => {
            if (e.target.classList.contains('table-input')) {
                ...
                if (!isNaN(index) && state[listProperty] && state[listProperty][index]) {
                    state[listProperty][index][field] = field === 'quantity' || field === 'cost' ? parseFloat(value) || 0 : value;
                    renderFn();
                }
            }
        });
        ...
        tbody.addEventListener('click', e => {
            ...
            if (btn && btn.classList.contains('danger')) {
                ...
                if (!isNaN(index) && state[listProperty] && state[listProperty][index]) {
                    state[listProperty].splice(index, 1);
                    renderFn();
                }
            }
        });
    ...
    function renderPaymentList() {
        ...
        if (!supplierCode || state.invoiceModalSelections.size === 0) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;">No invoices selected</td></tr>`;
            document.getElementById('payment-total-amount').textContent = '0.00 EGP';
            return;
        }
        ...
        let totalAmount = 0;
        state.invoiceModalSelections.forEach(invoiceNumber => {
            const invoice = supplierInvoices[invoiceNumber];
            if (invoice && invoice.balance > 0) {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${invoice.number}</td>
                    <td>${new Date(invoice.date).toLocaleDateString()}</td>
                    <td>${invoice.balance.toFixed(2)} EGP</td>
                    <td><input type="number" class="table-input payment-amount-input" data-invoice="${invoice.number}" value="${invoice.balance.toFixed(2)}" min="0" max="${invoice.balance}" step="0.01"></td>
                `;
        ...
        document.getElementById('payment-total-amount').textContent = `${totalAmount.toFixed(2)} EGP`;
        
        if (tbody.children.length === 0) {
            tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;">No valid invoices selected</td></tr>`;
        }
    }

    function openEditModal(type, id) {
        const record = findByKey(state[type + 's'], type === 'user' ? 'Username' : (type === 'item' ? 'code' : type + 'Code'), id);
        
        editModalTitle.textContent = id ? _t(`edit_${type}`) : _t(`add_new_${type}`);
            ...
            case 'item':
                formHtml = `
                    <div class="form-group">
                        <label for="edit-item-code">${_t('item_code')}</label>
                        <input type="text" id="edit-item-code" name="code" value="${record?.code || ''}" ${id ? 'readonly' : 'required'}>
                    </div>
                    <div class="form-group">
                        <label for="edit-item-name">${_t('item_name')}</label>
                        <input type="text" id="edit-item-name" name="name" value="${record?.name || ''}" required>
                    </div>
                    <div class="form-group-checkbox">
                        <input type="checkbox" id="edit-is-sub-item-toggle" ${record?.ParentItemCode ? 'checked' : ''}>
                        <label for="edit-is-sub-item-toggle">${_t('is_sub_item')}</label>
                    </div>
                    <div class="form-group" id="edit-parent-item-group" style="display: ${record?.ParentItemCode ? 'block' : 'none'}">
                        <label for="edit-parent-item-code">${_t('parent_item')}</label>
                        <select id="edit-parent-item-code" name="ParentItemCode">
                            <option value="">${_t('select_parent_item')}</option>
                            ${(state.items || []).filter(i => !i.ParentItemCode).map(i => 
                                `<option value="${i.code}" ${record?.ParentItemCode === i.code ? 'selected' : ''}>${i.name}</option>`
                            ).join('')}
                        </select>
                    </div>
                    <div class="form-group" id="edit-item-cost-group" style="display: ${record?.ParentItemCode ? 'none' : 'block'}">
                        <label for="edit-item-cost">${_t('default_cost')}</label>
                        <input type="number" id="edit-item-cost" name="cost" value="${record?.cost || 0}" min="0" step="0.01" ${record?.ParentItemCode ? 'readonly' : ''}>
                    </div>
                `;
                break;
            case 'supplier':
                formHtml = `
                    <div class="form-group">
                        <label for="edit-supplier-code">${_t('supplier_code')}</label>
                        <input type="text" id="edit-supplier-code" name="supplierCode" value="${record?.supplierCode || ''}" ${id ? 'readonly' : 'required'}>
                    </div>
                    <div class="form-group">
                        <label for="edit-supplier-name">${_t('supplier_name')}</label>
                        <input type="text" id="edit-supplier-name" name="name" value="${record?.name || ''}" required>
                    </div>
                `;
                break;
            case 'branch':
                formHtml = `
                    <div class="form-group">
                        <label for="edit-branch-code">${_t('branch_code')}</label>
                        <input type="text" id="edit-branch-code" name="branchCode" value="${record?.branchCode || ''}" ${id ? 'readonly' : 'required'}>
                    </div>
                    <div class="form-group">
                        <label for="edit-branch-name">${_t('branch_name')}</label>
                        <input type="text" id="edit-branch-name" name="branchName" value="${record?.branchName || ''}" required>
                    </div>
                `;
                break;
        }
        
        formHtml += `<div class="form-actions"><button type="submit" class="primary">${_t('save_changes')}</button><button type="button" class="secondary modal-cancel">${_t('cancel')}</button></div>`;
        
        editModalBody.innerHTML = formHtml;
        editModal.classList.add('active');
        
        // Add event listener for sub-item toggle
        const toggle = document.getElementById('edit-is-sub-item-toggle');
        if (toggle) {
            toggle.addEventListener('change', function() {
                document.getElementById('edit-parent-item-group').style.display = this.checked ? 'block' : 'none';
                document.getElementById('edit-item-cost-group').style.display = this.checked ? 'none' : 'block';
                if (this.checked) {
