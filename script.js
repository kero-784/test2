...
.sidebar-nav { list-style: none; padding: 0 16px; margin: 0; flex-grow: 1; overflow-y: auto; display: flex; flex-direction: column; }
...
.pending-requests-widget { display: flex; align-items: center; gap: 8px; background-color: var(--warning-color); color: white; padding: 6px 12px; border-radius: var(--border-radius); font-weight: 600; font-size: 14px; margin-left: 24px; }
.pending-requests-widget .icon { width: 20px; height: 20px; }
.user-branch-display {
    margin-left: auto; /* Pushes this to the very right */
...
.modal-content { background-color: white; padding: 0; border-radius: 16px; width: 90%; max-width: 800px; max-height: 90vh; display: flex; flex-direction: column; box-shadow: var(--shadow-lg); overflow: hidden; transform: scale(0.95); animation: modal-pop-in 0.3s ease-out forwards; }
@keyframes modal-pop-in { to { transform: scale(1); } }
.modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid var(--border-color); padding: 20px 24px; }
...
.modal-footer { display: flex; justify-content: flex-end; gap: 12px; padding: 20px 24px; border-top: 1px solid var(--border-color); background-color: #F7F8FC; }

/* --- Component-Specific Styles --- */
#table-receive-list input, #table-transfer-list input, #table-issue-list input, #table-po-list input, #table-return-list input, #table-request-list input { padding: 8px; font-size: 14px; max-width: 120px; background-color: white; }
...
.status-pending, .status-intransit { background-color: #eaf2fc; color: var(--primary-dark); }
...
.permissions-grid { grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); }
