...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwVZTFxYrxoGn0kNbwUMwDiBmRc-0KhQjB7V2LkaMUnEn02h6wpsNs8leL08xPYzfo6/exec';
...
// PART 5 of 5: EVENT LISTENERS & INITIALIZATION
    ...
    // All the render functions (renderUserManagementUI, renderPendingTransfers, etc.) go here...
    function attachEventListeners() {
        // Main nav and login/logout
        ...
        // Modal general event listeners are attached in Part 2...

        // Form Submissions
        document.getElementById('form-add-item').addEventListener('submit', async e => { e.preventDefault(); const btn = e.target.querySelector('button'); const data = { code: e.target.elements['item-code'].value, name: e.target.elements['item-name'].value, category: e.target.elements['item-category'].value, barcode: e.target.elements['item-barcode'].value, unit: e.target.elements['item-unit'].value, supplierCode: e.target.elements['item-supplier'].value, cost: e.target.elements['item-cost'].value }; const result = await postData('addItem', data, btn); if (result) { showToast('Item added!', 'success'); e.target.reset(); reloadDataAndRefreshUI(); } });
        //... add all other form submission listeners here
        document.getElementById('btn-submit-po').addEventListener('click', async e => { /* PO submission logic */ });
        document.getElementById('btn-submit-receive-batch').addEventListener('click', async e => { /* Receive submission logic */ });
        document.getElementById('btn-submit-request-supplies').addEventListener('click', async e => { /* Request Supplies submission logic */ });
        document.getElementById('btn-submit-report-shortage').addEventListener('click', async e => { /* Shortage Report submission logic */ });

        // Table interaction listeners (for dynamic inputs, delete buttons)
        document.getElementById('table-po-list').addEventListener('input', e => { if (e.target.dataset.field) { state.currentPOList[e.target.dataset.index][e.target.dataset.field] = parseFloat(e.target.value) || 0; } updatePOGrandTotal(); });
        document.getElementById('table-po-list').addEventListener('click', e => { if(e.target.classList.contains('danger')) { state.currentPOList.splice(e.target.dataset.index, 1); renderPOListTable(); } });
        // ... add all other table interaction listeners here
    }
