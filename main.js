...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- SESSION RESTORE (Prevents logout on refresh) ---
    ...
    async function login(u, p) {
        ...
        try {
            ...
            if (d.status !== 'error' && d.user) {
                ...
                // Persist Session
                sessionStorage.setItem('meatUser', u);
                sessionStorage.setItem('meatPass', p);
    ...
    document.querySelectorAll('#main-nav a').forEach(l => l.addEventListener('click', e => { e.preventDefault(); if(l.id==='btn-logout') { sessionStorage.clear(); location.reload(); } show(l.dataset.view); }));
