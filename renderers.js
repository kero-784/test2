...
export function renderReceiveListTable() { 
    renderDynamicListTable('table-receive-list', state.currentReceiveList, [ { type: 'text', key: 'itemCode' }, { type: 'text', key: 'itemName' }, { type: 'number_input', key: 'quantity' }, { type: 'cost_input', key: 'cost' }, { type: 'calculated', calculator: item => formatCurrency((parseFloat(item.quantity)||0)*(parseFloat(item.cost)||0)) } ], 'no_items_selected_toast', () => { document.getElementById('receive-grand-total').textContent = formatCurrency(state.currentReceiveList.reduce((a, i) => a + ((i.quantity||0)*(i.cost||0)), 0)); }); 
}
export function renderButcheryListTable() {
    renderDynamicListTable('table-butchery-children', state.currentButcheryList, [ { type: 'text', key: 'itemCode' }, { type: 'text', key: 'itemName' }, { type: 'number_input', key: 'quantity', step: 0.001 } ], 'no_items_selected_toast', () => {
        ...
        if(pQty > 0 && el) { const pct = (total/pQty)*100; el.textContent = pct.toFixed(1)+'%'; el.style.color = (pct>102||pct<50) ? 'red' : 'green'; }
    });
}
export function renderTransferListTable() { renderDynamicListTable('table-transfer-list', state.currentTransferList, [{ type: 'text', key: 'itemCode' }, { type: 'text', key: 'itemName' }, { type: 'available_stock', branchSelectId: 'transfer-from-branch' }, { type: 'number_input', key: 'quantity' }], 'no_items_selected_toast', () => { document.getElementById('transfer-grand-total').textContent = state.currentTransferList.reduce((a, i) => a + (parseFloat(i.quantity)||0), 0).toFixed(3); }); }
export function renderReturnListTable() { renderDynamicListTable('table-return-list', state.currentReturnList, [{ type: 'text', key: 'itemCode' }, { type: 'text', key: 'itemName' }, { type: 'available_stock', branchSelectId: 'return-branch' }, { type: 'number_input', key: 'quantity' }, { type: 'cost_input', key: 'cost' }], 'no_items_selected_toast', () => { document.getElementById('return-grand-total').textContent = formatCurrency(state.currentReturnList.reduce((a, i) => a + ((i.quantity||0)*(i.cost||0)), 0)); }); }
export function renderPOListTable() { renderDynamicListTable('table-po-list', state.currentPOList, [{ type: 'text', key: 'itemCode' }, { type: 'text', key: 'itemName' }, { type: 'number_input', key: 'quantity' }, { type: 'cost_input', key: 'cost' }, { type: 'calculated', calculator: item => formatCurrency((parseFloat(item.quantity)||0)*(parseFloat(item.cost)||0)) }], 'no_items_selected_toast', () => { document.getElementById('po-grand-total').textContent = formatCurrency(state.currentPOList.reduce((a, i) => a + ((i.quantity||0)*(i.cost||0)), 0)); }); }
export function renderPOEditListTable() { renderDynamicListTable('table-edit-po-list', state.currentEditingPOList, [{ type: 'text', key: 'itemCode' }, { type: 'text', key: 'itemName' }, { type: 'number_input', key: 'quantity' }, { type: 'cost_input', key: 'cost' }, { type: 'calculated', calculator: item => formatCurrency((parseFloat(item.quantity)||0)*(parseFloat(item.cost)||0)) }], 'no_items_selected_toast', () => { document.getElementById('edit-po-grand-total').textContent = formatCurrency(state.currentEditingPOList.reduce((a, i) => a + ((i.quantity||0)*(i.cost||0)), 0)); }); }
...
export function renderTransactionHistory(filters = {}) {
    ...
    // if (!userCan('viewAllBranches')) { tx = tx.filter(t => t.branchCode === state.currentUser.AssignedBranchCode); }
    if(tx.length === 0) { tbody.innerHTML = '<tr><td colspan="6">No data</td></tr>'; return; }
    tx.sort((a,b)=>new Date(b.date)-new Date(a.date)).forEach(t => {
        const details = t.type === 'receive' ? `GRN: ${t.invoiceNumber}` : (t.type.includes('production') ? 'Butchery' : t.type);
...
export function renderPendingFinancials() {
    ...
    const groups = {};
    pending.forEach(t => { if(!groups[t.batchId]) groups[t.batchId]={...t, val:0}; groups[t.batchId].val += (t.quantity*t.cost); });
    Object.values(groups).forEach(t => {
        tbody.innerHTML += `<tr><td>${formatDate(t.date)}</td><td>${t.type}</td><td>${t.batchId}</td><td>${t.invoiceNumber}</td><td>${formatCurrency(t.val)}</td><td><button class="primary small btn-approve-financial" data-id="${t.batchId}" data-type="receive">Approve</button></td></tr>`;
    });
}
