...
const finState = { initialized: false, invoiceSelections: new Set() };
...
function initFinancialsModule() {
    ...
    console.log("Financials Module Loaded");
}
...
function injectFinancialsUI() {
    ...
    if (sidebar && !document.getElementById('nav-financials-link')) {
        ...
        // LOGIC: Insert AFTER 'Purchasing'
        const purchBtn = sidebar.querySelector('a[data-view="purchasing"]')?.parentElement;
        if (purchBtn) {
            purchBtn.after(li);
        } else {
            const masterBtn = sidebar.querySelector('a[data-view="master-data"]')?.parentElement;
            if(masterBtn) sidebar.insertBefore(li, masterBtn);
        }
    }
    ...
    if (mainContent && !document.getElementById('view-financials-ext')) {
        ...
        viewDiv.innerHTML = `
            ...
            <div id="tab-fin-payment" class="fin-tab" style="display:block;">
                <div class="card"><h2>Record a Payment</h2><form id="ext-form-payment"><div class="form-grid"><div class="form-group"><label>Supplier</label><select id="ext-pay-supplier" required></select></div><div class="form-group"><label>Select Invoices</label><button type="button" id="ext-btn-sel-inv" class="secondary" style="width: 100%;">Select...</button></div><div class="form-group"><label>Method</label><select id="ext-pay-method" required><option value="Cash">Cash</option><option value="Bank Transfer">Bank Transfer</option><option value="Cheque">Cheque</option><option value="Credit">Credit</option></select></div></div><div id="ext-pay-list-container" class="report-area" style="display:none; margin-top:24px;"><table id="ext-pay-table"><thead><tr><th>Invoice #</th><th>Due</th><th>Pay</th></tr></thead><tbody></tbody><tfoot><tr><td colspan="2">Total:</td><td id="ext-pay-total">0.00</td></tr></tfoot></table></div><button type="submit" class="primary" style="margin-top: 24px; width:100%;">Submit Payment</button></form></div>
            </div>
            <div id="tab-fin-statement" class="fin-tab" style="display:none;">
                <div class="card"><div class="report-generator-controls"><select id="ext-stmt-supplier"></select><input type="date" id="ext-stmt-start"><input type="date" id="ext-stmt-end"><button id="ext-btn-gen-stmt" class="primary small">Generate</button></div><div id="ext-stmt-results" class="printable-area" style="display: none;"></div></div>
            </div>
            <div id="ext-inv-modal" class="modal-overlay">
                <div class="modal-content"><div class="modal-header"><h2>Select Invoices</h2><button class="close-button" id="ext-inv-close">×</button></div><div class="modal-body"><div id="ext-inv-list" class="modal-item-list"></div></div><div class="modal-footer"><button class="primary" id="ext-inv-confirm">Confirm Selection</button></div></div>
            </div>
        `;
...
function attachFinancialListeners() {
    ...
    document.getElementById('ext-inv-confirm')?.addEventListener('click', () => { document.getElementById('ext-inv-modal').classList.remove('active'); renderPaymentTable(); });
    document.getElementById('ext-inv-list')?.addEventListener('change', (e) => { if(e.target.type === 'checkbox') { const num = e.target.dataset.number; e.target.checked ? finState.invoiceSelections.add(num) : finState.invoiceSelections.delete(num); } });
    document.getElementById('ext-form-payment')?.addEventListener('submit', handleSubmitPayment);
    document.getElementById('ext-btn-gen-stmt')?.addEventListener('click', () => { renderSupplierStatement(document.getElementById('ext-stmt-supplier').value, document.getElementById('ext-stmt-start').value, document.getElementById('ext-stmt-end').value); });
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if(btn.classList.contains('btn-pay-supplier')) {
            ...
            setTimeout(() => { document.getElementById('ext-pay-supplier').value = supCode; openInvoiceModal(); }, 100);
        }
        if(btn.classList.contains('btn-print-voucher')) {
            const pid = btn.dataset.id;
            const lines = state.payments.filter(p => p.paymentId === pid);
            if(lines.length > 0) Documents.generatePaymentVoucher({ supplierCode: lines[0].supplierCode, date: lines[0].date, method: lines[0].method, totalAmount: lines.reduce((s,x)=>s+x.amount,0), payments: lines });
        }
        if(btn.id === 'ext-btn-export-stmt') {
             const supName = document.querySelector('#ext-stmt-results h3')?.innerText || 'Statement';
             if(window.XLSX) { const tbl = document.getElementById('table-supplier-statement'); const wb = XLSX.utils.table_to_book(tbl); XLSX.writeFile(wb, `${supName}.xlsx`); }
        }
    });
}
...
function openInvoiceModal() {
    ...
    else { unpaid.forEach(inv => { const checked = finState.invoiceSelections.has(inv.number) ? 'checked' : ''; list.innerHTML += `<div class="modal-item"><input type="checkbox" id="inv-${inv.number}" data-number="${inv.number}" ${checked}><label for="inv-${inv.number}"><strong>${inv.number}</strong><br><small>Due: ${formatCurrency(inv.balance)}</small></label></div>`; }); }
...
function renderPaymentTable() {
    ...
    finState.invoiceSelections.forEach(num => {
        ...
        if(inv) { total += inv.balance; tbody.innerHTML += `<tr><td>${inv.number}</td><td>${formatCurrency(inv.balance)}</td><td><input type="number" class="table-input ext-pay-amt" data-inv="${inv.number}" value="${inv.balance.toFixed(2)}"></td></tr>`; }
    });
...
function renderSupplierStatement(code, d1, d2) {
    ...
    container.innerHTML = `<div class="printable-document"><h3>Statement: ${sup.name}</h3><table id="table-supplier-statement"><thead><tr><th>Date</th><th>Type</th><th>Ref</th><th>Debit</th><th>Credit</th><th>Balance</th><th>Action</th></tr></thead><tbody>${html}</tbody></table><div style="margin-top:20px; text-align:right;"><button class="secondary" id="ext-btn-export-stmt">Export Excel</button><button class="primary btn-pay-supplier" data-supplier="${sup.supplierCode}">Pay Supplier</button></div></div>`;
    container.style.display = 'block';
}

const interval = setInterval(() => { if (state.currentUser && document.getElementById('main-nav')) { initFinancialsModule(); clearInterval(interval); } }, 500);
